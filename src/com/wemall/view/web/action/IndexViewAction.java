...
public class IndexViewAction {
    ...
    public ModelAndView index(HttpServletRequest request, HttpServletResponse response){
        ...
        params.put("validity_time", df.format(new Date()));
        List GoodsValidity_time = this.goodsService.query("select obj from Goods obj where obj.validity_time<:validity_time ", params, 0, 5);
        if (GoodsValidity_time.size()>0){
        	for (int i = 0; i < GoodsValidity_time.size(); i++) {
        		Goods goodsupd=(Goods) GoodsValidity_time.get(i);
        		int Goods_status=1;
        		goodsupd.setGoods_status(Goods_status);
        		this.goodsService.update(goodsupd);
			}
        }
    ...
    public ModelAndView find_pws(HttpServletRequest request, HttpServletResponse response, String userName,String code, String password,String repassword){
        ...
        for(GoodsFloor info : floor.getChilds()){
            ...
            for(Goods goods : this.gf_tools.generic_goods(info.getGf_gc_goods())){
                if(goods != null){
                    ...
                    try{
                    	String name = SecurityUserHolder.getCurrentUser().getUserName();
                    	sb.append("</span></li><li><span class=\"hui2\">商城价：</span><strong class=\"red\">￥").append(goods.getGoods_current_price());
                    }catch(Exception e){
                    	sb.append("</span></li><li><span class=\"hui2\">商城价：</span><strong class=\"red\">").append("请登录!");
                    }
