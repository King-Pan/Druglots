...
public class AccountBuyerAction {
    ...
    public ModelAndView account(HttpServletRequest request, HttpServletResponse response){
		...
		try {
			...
			String Load = hc.load("http://127.0.0.1:8081/ssm_project/selectlw", "userId="+userid);
    ...
    public ModelAndView account_save(HttpServletRequest request, HttpServletResponse response, String area_id, String birthday,String trueName,String areaId,String enterpriseName){
 ...
 //       user.setAddress(area.getParent().getParent().getAreaName()+area.getParent().getAreaName()+area.getAreaName());
        //存一份到首营认证表
        //查首营认证消息
        HttpClass hc = new HttpClass();
		try {
			String Load = hc.load("http://127.0.0.1:8081/ssm_project/selectlw", "userId="+SecurityUserHolder.getCurrentUser().getId());
			Authentication authentication = JSON.parseObject(Load, Authentication.class);
			authentication.setTrueName(trueName);
			authentication.setAreaId(areaId);
			authentication.setEnterpriseName(enterpriseName);
			authenticationService.update(authentication);
			
			 //存一份到user表
	        user.setTrueName(trueName);
	        user.setAddress(areaId);
	        user.setWW(enterpriseName);
	        this.userService.update(user);
		} catch (Exception e) {
			e.printStackTrace();
		 mv = new JModelAndView("error.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
		 mv.addObject("op_title", "个人信息修改失败");
    ...
    public ModelAndView account_password_save(HttpServletRequest request, HttpServletResponse response, String old_password, String new_password,String phonecode,String mobile) throws Exception {
        ModelAndView mv = new JModelAndView("success.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
        String wemall_view_type = CommUtil.null2String(request.getSession().getAttribute("wemall_view_type"));
        if((wemall_view_type != null) && (!wemall_view_type.equals("")) && (wemall_view_type.equals("wap"))){
        ...
        if (phone==null || !phone.equals(phonecode)) {
        	mv = new JModelAndView("error.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
    ...
    public ModelAndView account_mobile_save(HttpServletRequest request, HttpServletResponse response, String phonecode, String newMobile,String mobile) throws Exception {
        ModelAndView mv = new JModelAndView("success.html", this.configService
                                            .getSysConfig(), this.userConfigService.getUserConfig(), 1,
                                            request, response);
        //获取旧手机号
        ...
        mv = new JModelAndView("error.html", this.configService.getSysConfig(),
                               this.userConfigService.getUserConfig(), 1, request,
                               response);
        mv.addObject("op_title", "验证码错误，手机修改失败");
        mv.addObject("url", CommUtil.getURL(request) +
        ...
        return mv;
          ...
          User user = this.userService.getObjById(
                SecurityUserHolder.getCurrentUser().getId());
          //Assert.isTrue(user.getMobile().equals(mobile), "手机号已存在!!!");
          user.setMobile(newMobile);
          this.userService.update(user);
          mv.addObject("op_title", "手机号修改成功");
          mv.addObject("url", CommUtil.getURL(request) +
                  "/buyer/account_mobile.htm");
        return mv;
    }
     ...
     * 用户修改支付密码页面
     */
    @RequestMapping("/buyer/account_paypassword.htm")
    public ModelAndView account_paypassword(HttpServletRequest request, HttpServletResponse response){
        ModelAndView mv = new JModelAndView("user/default/usercenter/account_paypassword.html", this.configService.getSysConfig(),
                                            this.userConfigService.getUserConfig(), 0, request, response);
        String wemall_view_type = CommUtil.null2String(request.getSession().getAttribute("wemall_view_type"));
        if((wemall_view_type != null) && (!wemall_view_type.equals("")) && (wemall_view_type.equals("wap"))){
            mv = new JModelAndView("wap/account_paypassword.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
        }
        //获取当前登录人
        User user = SecurityUserHolder.getCurrentUser();
        //获取手机号
     ...
     * 用户修改支付密码
     */
    @RequestMapping("/buyer/fixpaypwd.htm")
    public ModelAndView fixpaypwd(HttpServletRequest request, HttpServletResponse response, String old_password, String new_password,String phonecode,String mobile) throws Exception {
        ModelAndView mv = new JModelAndView("success.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
        String wemall_view_type = CommUtil.null2String(request.getSession().getAttribute("wemall_view_type"));
        if((wemall_view_type != null) && (!wemall_view_type.equals("")) && (wemall_view_type.equals("wap"))){
            mv = new JModelAndView("wap/success.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
        ...
        if (phone==null || !phone.equals(phonecode)) {
        	mv = new JModelAndView("error.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
            mv.addObject("op_title", "验证码输入错误");
            mv.addObject("url", CommUtil.getURL(request) + "/buyer/account_paypassword.htm");
            return mv;
		}
        
        User user = this.userService.getObjById(
                        SecurityUserHolder.getCurrentUser().getId());
        if (user.getPayPwd().equals(
                    Md5Encrypt.md5(old_password).toLowerCase())){
            user.setPayPwd(Md5Encrypt.md5(new_password).toLowerCase());
            boolean ret = this.userService.update(user);
            if ((ret) && (this.configService.getSysConfig().isUc_bbs())){
                UCClient uc = new UCClient();
                String str = uc.uc_user_edit(user.getUsername(),
