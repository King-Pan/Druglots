    ...
    public ModelAndView add_goods_finish(HttpServletRequest request,
        ...
        if (goods_session1.equals("")){
            ...
            String saveFilePathName = request.getSession().getServletContext()
                      .getRealPath("/") +
                      uploadFilePath + File.separator + "good_main_photo";
            Map map1 = new HashMap();
            try {
				map1=CommUtil.saveFileToServer(request, "uploadImg", saveFilePathName, null, null);
				if (map1.get("fileName") != "") {
					Accessory goodsimgacc=new Accessory();
					goodsimgacc = new Accessory();
					goodsimgacc.setName(CommUtil.null2String(map1.get("fileName")));
					goodsimgacc.setExt(CommUtil.null2String(map1.get("mime")));
                    goodsimgacc.setSize(CommUtil.null2Float(map1.get("fileSize")));
                    goodsimgacc.setPath(uploadFilePath + "/good_main_photo");
                    goodsimgacc.setWidth(CommUtil.null2Int(map1.get("width")));
                    goodsimgacc.setHeight(CommUtil.null2Int(map1.get("height")));
                    goodsimgacc.setAddTime(new Date());
                    this.accessoryService.save(goodsimgacc);
                    goods.setGoods_main_photo(goodsimgacc);
				}
			} catch (IOException e) {
    ...
    public String goods_sale(HttpServletRequest request,
        ...
        for (String id : ids){
            if (!id.equals("")){
                ...
                //判断时间是否正确
                String validity_time = goods.getValidity_time();
                Date olddate=null;
                Date newdate = new Date();
                SimpleDateFormat formatter=new SimpleDateFormat("yyyy-MM-dd"); 
                try {
                	olddate=formatter.parse(validity_time);
