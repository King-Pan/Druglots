...
public class UCTools {
    ...
    public boolean addTemii_OpeningStatus(String userName,String certificate,String license,String GSPCertificate,String letterOfInstruction,String CopyOfIdCard){
        boolean ret = true;
        Connection conn = getConnection();
        try {
            Statement stmt = conn.createStatement();
            String sql = "insert into  IOU (`userName`,`certificate`,`license`,`GSPCertificate`,`letterOfInstruction`,`CopyOfIdCard`) value ('" +
                         userName + "','" + certificate + "','" + license + "','" +
                         GSPCertificate + "','"+letterOfInstruction+"','"+CopyOfIdCard+"')";
            ret = stmt.execute(sql);
        } catch (SQLException e){
            e.printStackTrace();
        } finally {
